...
int life_predict(unsigned char  *data);
...
int life_predict(unsigned char  *data)
{
	static unsigned char predict[unit];
	int i, same=1;
	for (i = 0; i < unit; i++)
	{
		if (abs(data[i] - predict[i])>thr)
		{ same = 0; break; }
	}
	if (same) printf("The predict has been verified!!!!!!!!!!!!!!!!!!!!!!!!!\n");
	i = rand() % unit;
	memcpy(predict, data, unit);
	predict[i] = MIN(predict[i] + thr/2, 255);
	return 0;
}
...
double life_compare_record(unsigned char  *data, particle *record, FILE* life, bool is_server)
{
	...
	while (!feof(life))
	{
	    ...
	    {
			if (abs(data[i] - past_life[i])>thr) { same = 0; break; }
	    }
		if (same)
		{
			break;
	...
	if (same)
	{
		...
		sprintf(record->remark, "Original User:%s",past_record.UID);
		record->time[1] = past_record.time[0];
...
int life_save_record(unsigned char *data, particle *record, FILE* life_share, FILE* life_private, FILE *life_private_index, bool is_server)
{
	...
	else
	{
		...
		if (!record->is_repeat)
		{
			...
			fwrite(record, sizeof(particle), 1, life_private);
			//printf("Save a private particle!\n");
		}

		//always save record 
		fseek(life_private_index, 0, SEEK_END);
		fwrite(record, sizeof(particle), 1, life_private_index);
		//printf("Save a private record: UID:%s, VID:%s, PID:%s, GID:%s\n", record->UID, record->VID, record->PID, record->GID);
		return 0;
	}
}
...
int main(int argc, char** argv)
{
	...
	if (!strcmp(argv[1], "c"))
	{
		...
		for (;;)
		{
			...
			sprintf(record.remark, ".\\photo\\%s_%s.jpg", record.UID, record.VID);
			...
			//pedict life
			life_predict((unsigned char*)result.data);
			...
			if (record.is_repeat)
			{
				timeinfo = localtime((time_t*)&(record.time[1]));
		     	printf("Its first appearance date/time is: %s %s\n", asctime(timeinfo), record.remark);
			}
